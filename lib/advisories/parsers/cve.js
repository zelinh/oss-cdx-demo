        ...
        json.affects?.vendor?.vendor_data?.forEach?.(vd => {
            ...
            vd.product?.product_data?.forEach?.(pd => {
                ...
                pd.version?.version_data?.forEach?.(vd => {
                    ...
                    if (version && !versions.includes(version) && version !== productName) {
                ...
                if (versions.length !== 0) {
                    const versionsList = tokenize.byLength(versions, versions.join(' || ').length);
                    versionsList.forEach(versionsItem => {
                        const versionsItemJoined = versionsItem.join(' || ');
                        if (!productsMap.includes(`${res.vendor}//${res.name}//${versionsItemJoined}`)) {
                            products.push({ ...res, version: versionsItemJoined });
                            productsMap.push(`${res.vendor}//${res.name}//${versionsItemJoined}`);
                        }
                    });
                }
            });
