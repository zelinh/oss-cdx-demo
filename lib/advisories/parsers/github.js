            ...
            json.affected.forEach(item => {
                ...
                if (Array.isArray(item.ranges)) {
                    item.ranges.forEach(range => {
                        ...
                        if (res.length) versions.add(res.join(' '));
                    });
                }

                if (Array.isArray(item.versions)) item.versions.forEach(version => versions.add(version));

                if (
                    versions.size === 0 &&
                    item.database_specific?.last_known_affected_version_range
                ) versions.add(item.database_specific.last_known_affected_version_range);
                /*
                ...
                if (!versions.size) {
                    if (foundIntroducedZero) versions.add('*');
                    else throw 'NO_VERSIONS';
                }
                product.version = Array.from(versions).join(' || ');
