    ...
    cache: {
        ...
        save: async (key, data) => {
            ...
            for await (const _d of _data) {
                ...
                await fs.outputJSON(path.join(CACHE_ROOT, key, `${_d.id}.json`), _d);
            }
        },
        ...
        finalize: async (cacheDirs, func) => {
            ...
            while (true) {
                const loc = list.shift();
                console.log(`Catalogues to go: ${list.length}`);
                ...
                for await (let file of files) {
                    ...
                    for await (let other of list) {
                        const otherFile = path.join(other, file);
