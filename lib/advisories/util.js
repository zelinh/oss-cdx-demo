    ...
    cache: {
        ...
        finalize: async (cacheDirs, func) => {
            ...
            for await (const [key, files] of idCollection) {
                ...
                // If product info is found from any other source than GSD, get rid of the GSD ones.
                /*
                if (Array.isArray(rec.products)) {
                    const qualityProducts = rec.products
                        .filter(product =>
                            Array.isArray(product.source)
                                ? (product.source.length > 1 || product.source[0] !== 'gsd')
                                : (product.source !== 'gsd')
                        );

                    if (qualityProducts.length !== rec.products.length) rec.products = qualityProducts;
                }
                 */
