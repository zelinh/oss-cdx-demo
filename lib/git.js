...
const getHistorical = async (repo, branch) => {
    ...
    try {
        ...
        (await git.raw(['log', '--since=\'2021-04-01T00:00:00\'', '--until=\'6 months ago\'', '--format=%ct#%H', branch]))?.toString?.()
        ...
        (await git.raw(['log', '--since=\'2 months ago\'', '--until=\'1 week ago\'', '--format=%ct#%H', branch]))?.toString?.()
