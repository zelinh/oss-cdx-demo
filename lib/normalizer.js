...
const tokenizeByLength = (list, currentSize, maxSize = 10000) => {
    const partSize = Math.floor(list.length * maxSize / currentSize);
    const numParts = Math.ceil(list.length / partSize);
    const result = [];
    for (let i = 0; i < numParts; i++) {
        result.push(list.slice(i * partSize, (i + 1) * partSize));
    }
