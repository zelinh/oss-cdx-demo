...
const initIndices = async () => {
    ...
    //await OpenSearch.pointAlias('sbom-all', 'raw-sbom-000001');
    //await OpenSearch.pointAlias('vulnerabilities-all', 'raw-vulnerabilities-000001');
...
const search = async (index, body, size = 100, transform = null, loop = false) => {
    ...
    for await (const scrollIdList of chunks(scrollIds, 20)) {
        try {
            await client.clearScroll({ scroll_id: scrollIdList });
        } catch (ex) {}
    }
