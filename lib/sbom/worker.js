...
const run = async project => {
    ...
    try {
            ...
            const res = {
                ...
                origin: relativeRoot
                    ? Array.isArray(rec._src)
                        ? rec._src.map(src =>
                            Array.isArray(src)
                                ? [path.join(relativeRoot, src[0]), ...src.slice(1)]
                                : path.join(relativeRoot, src)
                        )
        ...
        await OpenSearch.updateProgressState(lockId, 'sbom', 'failed');
        throw ex;
    } finally {
        console.log(`[${logKey}]\tReleasing lock ${lockId}`);
        await OpenSearch.updateProgressState(lockId, 'sbom', 'done');
    }
};
