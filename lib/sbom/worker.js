...
const run = async project => {
    ...
    if (!process.env.CDX_HISTORICAL && await OpenSearch.indexExists(indexName)) {
        console.log(`[${logKey}]\tSkipping #${project.hash}`);
    } else {
        // Execution timestamp
        const now = Date.now();
        const { dir } = await git.clone(project.repo, project.tag, project.hash);
        ...
        // Save records
        if (process.env.CDX_HISTORICAL) {
            console.log(`[${logKey}]\tIndex historical...`);
            const allIndexExists = await OpenSearch.indexExists(`${prefixes.sbom}all`);
            if (allIndexExists) await OpenSearch.deleteDocs(`${prefixes.sbom}all`, {project: project.name, tag: project.tag, hash: project.hash});
    ...
    await OpenSearch.pointAlias(normalizer.getAliasName(project), indexName, true);
};
