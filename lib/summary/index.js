...
const getWeekSummary = week => week.projects.buckets.reduce((result, bucket) => {
    result[bucket.key] = bucket.scanned.buckets[0].committed.buckets[0].severities.buckets.reduce((result, severity) => {
        result[HIGH_SEVS.includes(severity.key) ? 'high' : 'low'] += severity.doc_count;
        return result;
    }, { high: 0, low: 0, date: bucket.scanned.buckets[0].key_as_string });
    return result;
}, {});

const create = async () => {
    const [currentWeek, previousWeek] = await awaitAll([
        OpenSearch.getAggregates(`${prefixes.vulnerabilities}all`, {
            ...
            query: {
                bool: {
                    filter: [
                        { range: { 'timestamp.scan': { gt: 'now-7d/d' } } },
        ...
        OpenSearch.getAggregates(`${prefixes.vulnerabilities}all`, {
            ...
            query: {
                bool: {
                    filter: [
                        { range: { 'timestamp.scan': { lte: 'now-7d/d' } } },
                        { term: { tag: { value: 'origin/main' } } },
                        { bool: { must_not: { match: { excluded: true } } } },
                    ]
                }
    ...
    for (const project of projects) {
        ...
        if (!currentWeekSummary[project.name] && !previousWeekSummary[project.name]) continue;

        result[project.name] = {
            currentWeek: currentWeekSummary[project.name],
            previousWeek: previousWeekSummary[project.name],
        };
    }
