...
const diffAggregates = (current = {}, prev = {}) => {
    ...
    const prevAliases = prev.aliases?.flat?.() || [];
    ...
    return {
        ...
        news: Array.from(current.aliases?.reduce?.((res, aliases) => {
            if (!aliases.some(alias => prevAliases.includes(alias))) res.add(aliases[0]);
            return res
        }, new Set()) || []),
...
const getDailyChanges = async (timestamp = Date.now(), daysApart = 1) => {
                            ...
                            aggs: {
                                vulnerabilities: {
                                    top_hits: {
                                        ...
                                        _source: { includes: ['count', 'timestamp', 'vulnerabilities.aliases', 'vulnerabilities.excluded'] },
                                        size: 1
                                    }
                                }
                            }
    ...
    const dayMinusZeroProjects = parseAggregates(dayMinusZero);
    const dayMinusOneProjects = parseAggregates(dayMinusOne);
