...
const parseDailyAggregates = data => {
    const map = new Map();
...
const getChanges = async (timestamp = Date.now(), daysApart = 1, limitedToMain = false) => {
    ...
    const dayMinusZeroProjects = parseDailyAggregates(dayMinusZero);
    const dayMinusOneProjects = parseDailyAggregates(dayMinusOne);
...
const getLatestChanges = async (limitedToMain = false) => {
    const queryMainTag = limitedToMain
        ? { query: { bool: { filter: [{ term: { 'project.tag': { value: 'origin/main' } } }] } } }
        : [];

    const latestAggregates = await OpenSearch.getAggregates(indices.scans, {
        aggs: {
            projects: {
                terms: { field: "project.name", order: { _key: 'asc' }, size: 10000 },
                aggs: {
    ...
    const latestProjects = parseLatestAggregates(latestAggregates);

    const result = [];
    for (const { name, disabled } of projects) {
        if (disabled) continue;

        const info = latestProjects.get(name);
        if (!info) {
            console.log(`No scans found for ${name}`);
            continue;
        }
        ...
        for (const tag of info.tags) {
            result.push({
                project: name,
                tag,
                ...diffAggregates(info.current.get?.(tag), info.prev.get?.(tag))
            });
        }
    }
...
const sendLatestNews = async (method = 'slack') => {
    ...
    const news = extractNews(await getLatestChanges(false));
