...
const checkProject = async (name, tag, hash) => {
    ...
    const excluded = await getExcludedAdvisoriesMap(timestamp.commit);
    ...
    await OpenSearch.indexDoc(indices.scans, {
        ...
        count,
        timestamp: {
            scan: timestamp.commit,
            commit: timestamp.commit
        }
    });
};

const getExcludedAdvisoriesMap = async timestamp => {
    const [
        excludedAdvisories,
        excludedRules
    ] = await awaitAll([
...
const run = async ({project, tag, hash} = {}) => {
    ...
    const projectAggs = await OpenSearch.getAggregates(indices.sboms, {
            aggs: {
                projects: {
                    ...
                    aggs: {
                        tags: {
                            ...
                            aggs: {
                                hash: {
                                    terms: { field: "project.hash", size: 10000 },
                                    aggs: {
                                        commit: {
                                            top_hits: {
    ...
    const projects = [];
    for (const { tags: { buckets: tagBuckets } } of projectAggs.projects.buckets) {
        for (const { hash: { buckets: hashBuckets } } of tagBuckets) {
            for (const { commit: { hits: { hits } } } of hashBuckets) {
    ...
    for (const { name, tag, hash } of projects) {
        ...
        await checkProject(name, tag, hash);
    }
};
