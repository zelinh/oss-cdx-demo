...
const checkProject = async (name, tag, hash, excluded) => {
    ...
    for await (const packages of chunks(packageMapKeys, 300)) {
        ...
        do {
            ...
            try {
                ...
                failed = true;
                if (++retries > 3) {
                    console.error('Failed to query advisories despite retrying');
                    throw ex;
                }

                console.warn(`Failed to query advisories; retrying in ${retries * 5}s...`);
    ...
    console.log(`Recorded vulnerabilities`);
};
...
const runPendingRescans = async () => {
    const [
        rescans,
        exclusionMap
    ] = await awaitAll([
        OpenSearch.search(indices.progress, {
            query: {
                bool: {
                    filter: [
                        { term: { type: { value: 'scan' } } },
                        { term: { state: { value: 'queued' } } },
                    ]
                }
            },
            _source: ['project.name', 'project.tag', 'project.hash']
        }, 1000, null, true),
        getExcludedAdvisoriesMap()
    ]);

    let cnt = 0;
    let size = rescans.length;
    for (const { _id, _source: { project: { name, tag, hash } } } of rescans) {
        console.log(`Checking ${++cnt}/${size} ${name}@${tag} #${hash}...`);

        await OpenSearch.updateProgressState(_id, 'scan', 'started');

        await checkProject(name, tag, hash, exclusionMap);

        await OpenSearch.updateProgressState(_id, 'scan', 'done');
    }
};

const run = async ({name: project, tag, hash} = {}) => {
    const params = {};
    if (project || tag || hash) {
        params.query = {
            bool: {
    ...
    const [
        ...
        OpenSearch.getAggregates(indices.sboms, {
            aggs: {
                projects: {
    ...
    for (const { name, tag, hash } of projects) {
        ...
        let id = ids.get(`${name}@${tag}@${hash}`);
        if (id) await OpenSearch.updateProgressState(id, 'scan', 'started');
        else id = await OpenSearch.lockProgressState({name, tag, hash}, 'scan', 'started');

        await checkProject(name, tag, hash, exclusionMap);

        if (id) {
            console.log(`Marking progress as done for ${id}`);
            await OpenSearch.updateProgressState(id, 'scan', 'done');
        }
    }
};
